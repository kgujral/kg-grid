...
export class ToolPanelColumnGroupComp extends Component
  ...
  private static TEMPLATE = `<div class="ag-column-tool-panel-column-group">
  ...
  public init(): void {
    ...
    this.displayName = this.columnController.getDisplayNameForOriginalColumnGroup(
      null,
      this.columnGroup,
      "toolPanel"
    );
  ...
  private onChangeCommon(nextState: boolean): void {
    ...
    if (this.columnController.isPivotMode()) {
      ...
      const isAllowedColumn = (c: Column) =>
        !c.getColDef().lockVisible && !c.getColDef().suppressToolPanel;
  ...
  private workOutSelectedValue(): boolean | null {
    ...
    this.columnGroup.getLeafColumns().forEach((column: Column) => {
      ...
      const ignoredColumn =
        column.getColDef().lockVisible || column.getColDef().suppressToolPanel;
      if (ignoredColumn) {
        return null;
      }
